...
class BaseDataset(torchdata.Dataset):
    def __init__(self, list_sample, opt, max_sample=-1, split='train'):
        ...
        self.normalize = opt.audioNorm
        self.specAug = opt.specAug
        self.split = split
        self.seed = opt.seed
    ...
    def dummy_mix_frame_audio(self, N):
        frames = [None for n in range(N)]
        audios = [None for n in range(N)]
        mags = [None for n in range(N)]
        mags_shift = [None for n in range(N)]
        ...
        for n in range(N):
            ...
            audios[n] = torch.zeros(self.audLen)
            mags[n] = torch.zeros(1, self.HS, self.WS)
            mags_shift[n] = torch.zeros(1, self.HS, self.WS)
        return frames, audios, mags, mags_shift